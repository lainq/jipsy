...
const switchCaseStatement = (statement:any, discriminator:any, index:number):string => {
    ...
    return `${
        condition ? (index > 0 ? "elif" : "if") : "else"
    } ${
...
export const getSwitchStatementValue = (statement:any):string => {
    ...
    let cases = statement.cases.map((case_:any, index:number):string => {
